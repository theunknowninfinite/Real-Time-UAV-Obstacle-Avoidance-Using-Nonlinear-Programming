...
class OffboardControl(Node):
    ...
    def timer_callback(self) -> None:
        ...
        if self.vehicle_status.nav_state == VehicleStatus.NAVIGATION_STATE_OFFBOARD:
            ...
            if self.wp==0:
                ...
                # If near the wp
                if self.dist2wp(0.0, 0.0, -5.0) < 0.5:
                    self.counter+=1
                ...
                # If near the wp
                if self.dist2wp(5.0, 0.0, -5.0) < 0.5:
                    self.counter+=1
            
            # elif self.wp==2:
            #     # Publish (10, 0, -25)
            #     self.publish_position_setpoint(10.0, 0.0, -5.0)
            #     # If near the wp
    ...
    #     # Till the drone height is greater than the setpoint height, keep publishing (0, 0, setpoint_height)
    #     if self.vehicle_local_position.z > self.takeoff_height and self.vehicle_status.nav_state == VehicleStatus.NAVIGATION_STATE_OFFBOARD:
    #         self.publish_position_setpoint(0.0, 0.0, self.takeoff_height)

    #     # The moment the drone height is less than the setpoint height, land it
