...
class OffboardControl(Node):
    ...
    def __init__(self) -> None:
        ...
        # Create subscribers
        # Subscribe to the vehicle's local position
        self.vehicle_local_position_subscriber = self.create_subscription(
            VehicleLocalPosition, '/fmu/out/vehicle_local_position', self.vehicle_local_position_callback, qos_profile)
        # Subscribe to the vehicle's status
        self.vehicle_status_subscriber = self.create_subscription(
            VehicleStatus, '/fmu/out/vehicle_status', self.vehicle_status_callback, qos_profile)
        # Subscribe to the obstacle position topic
        self.obstacle_position_subscriber = self.create_subscription(
            Float32MultiArray, '/obstacle_point', self.obstacle_position_callback, qos_profile)
        ...
        self.goal_points = [(3.0, 0.0, -0.7), (0.0, 0.0, -0.7)]
        ...
        # Initialise variables for the obstacle position
        self.obstacle_x = -1.0
        self.obstacle_y = -1.0
        self.obstacle_z = -1.0
        ...
        self.trajectory_generator_timer = self.create_timer(self.dt, self.trajectory_generator)
    ...
    def obstacle_position_callback(self, msg):
        ...
        obstacle_x = msg.data[0]
        obstacle_y = msg.data[1]
        obstacle_z = msg.data[2]

        # If there is an obstacle in the environment, obtain the obstacle position in inertial frame
        if obstacle_x != -1 and obstacle_y != -1 and obstacle_z != -1:
            # Get the yaw of the drone
            yaw = self.vehicle_local_position.heading

            # First unrotate the obstacle position by the yaw angle
            yaw = -yaw
            self.obstacle_x = obstacle_x*np.cos(yaw) - obstacle_y*np.sin(yaw)
            self.obstacle_y = obstacle_x*np.sin(yaw) + obstacle_y*np.cos(yaw)
            ...
            # Rotate the obstacle position by the yaw angle
            self.obstacle_x = self.obstacle_x + x
            self.obstacle_y = self.obstacle_y + y
            self.obstacle_z = z

        # If there is no obstacle in the environment, set the obstacle position to -1
        else:
    ...
    def trajectory_generator(self):
        """
        Generate a trajectory for the drone to follow while avoiding obstacles.
        ...
        Using the obstacle position, the trajectory is generated such that the
        drone avoids the obstacle
        """

        # Print the obstacle position
        print('Obstacle Position: ', np.round(self.obstacle_x, 2),
                                     np.round(self.obstacle_y, 2),
                                     np.round(self.obstacle_z, 2))
        ...
        if self.ready and not self.waypoints and not self.goal_reached:
            ...
            # If the obstacle position is known, add the obstacle avoidance constraint
            if self.obstacle_x!=-1 and self.obstacle_y!=-1 and self.obstacle_z!=-1:
                X_obs = self.obstacle_x
                Y_obs = self.obstacle_y
                Z_obs = self.obstacle_z

                # Define the safety distance
                safety_distance = 1

                # Add the obstacle avoidance constraint
                m.Equation(((X - X_obs)**2 + (Y - Y_obs)**2 + (Z - Z_obs)**2)**0.5 >= safety_distance)
            ...
            # Print the predicted obstacle position
            print('Predicted Obstacle Position: ', self.obstacle_x, self.obstacle_y, self.obstacle_z)
            # Prompt user to start the mission
            choice = str(input('Proceed? (y/n): '))
    ...
    def controller(self) -> None:
        ...
        if self.vehicle_status.nav_state == VehicleStatus.NAVIGATION_STATE_OFFBOARD:
            ...
            x_local = np.round(self.vehicle_local_position.x, 2)
            y_local = np.round(self.vehicle_local_position.y, 2)
            z_local = np.round(self.vehicle_local_position.z, 2)
            yaw_local = np.round(self.vehicle_local_position.heading, 2)

            print(f"Current Position: {x_local, y_local, z_local, yaw_local}")
